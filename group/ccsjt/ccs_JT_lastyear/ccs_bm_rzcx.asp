...
sql1="SELECT fkmshuom,kmshuom FROM cwys_km WHERE depar='"& f &"' and nian='"&year(date())-1&"' order by sn"                                                                            
...
<!--
function xx(j,k,l,m,n,p) { //v3.0
   //s=depar.value ;
   //y=nian.value ;
   surl="ccs_bm_rzcx.asp?mnykm="+j+"&year1="+k+"&month1="+l+"&year2="+m+"&month2="+n+"&mnykmcode="+p;
        ...
        <tr>
          ...
          <td align="right">
            <table border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td><a href><img src="images/bmysglsm3.gif" width="138" height="28" border="0"></a></td>
              </tr>
            </table>
          </td>
        </tr>
...
<table width="771" border="0" cellspacing="0" cellpadding="0" height="90%">
 ...
 <TBODY>
  ...
  set objRst1.activeConnection=objConn
                  ...
                  objrst1.Source ="select distinct kmshuom,kmcode from cwys_km where depar = '"&f&"' and nian='"&year(date())-1&"' order by kmcode,kmshuom" 
  ...
  objRst.CursorType=3
  set objRst.activeConnection=objConn
                   objrst.Source ="select distinct kmshuom,kmcode,fkmshuom,fkmcode from cwys_km where depar = '"&f&"' and kmshuom='"&trim(objrst1("kmshuom"))&"' and nian='"&year(date())-1&"' order by kmcode,kmshuom,fkmcode,fkmshuom" 
                  ...
                  <TR>
                    <TD style="PADDING-LEFT: 00px" height=23> <A title=资料册 
                        href="ccs_bm_rzcx.asp?km=<%=trim(objrst("fkmshuom"))%>&km1=<%=trim(objrst("fkmcode"))%>&km2=<%=trim(objrst("kmshuom"))%>" 
    ...
    <td width="626" valign="top"> 
      <table style="BORDER-RIGHT: #4983a0 1px solid; BORDER-TOP: #4983a0 1px solid; BORDER-LEFT: #4983a0 1px solid; BORDER-BOTTOM: #4983a0 1px solid" height="100%" cellSpacing="0" cellPadding="0" width="814" border="0">
        <tbody>
        <tr>
          <td style="PADDING-LEFT: 10px; PADDING-TOP: 2px" bgColor="#4e4c71" height="20" width="802"><font style="COLOR: #ffffff; FONT-FAMILY: Webdings">4</font><font color="#ffffff" class="px14">&nbsp;日志查询</font> </td></tr>
        <tr>
   ...
   <tr>
    <td align="center" ><font class="px12" color="black">您目前进入的是<font color=blue><%=km2%>(<%=km%>)</font><font color=red>日志查询</font>界面</td></tr>
    <tr>
    <td align="right" >　</td></tr>
...
<input type="submit" value="查询" id="submit1" name="submit1" onclick="javascript:xx(mnykm.value,year1.value,month1.value,year2.value,month2.value,mnykmcode.value)">
...
%>
 ...
 <table cellSpacing="1" cellPadding="0" width="800">
       ...
       <tr bgColor="c0c0c0" height="20">
                ...
                <td align="middle"  ><font color=black class=px12><b>是否核销</b></td>
                <td align="middle"  ><font color=black class=px12><b>修改记录号</b></td>
...
sqlq="select * from cwys_bmglrz where mnykmcode='"&fkmcode1&"' and mnytime>='"&date1&"' and mnytime<='"&date2&"' and mnydepm='"&f&"' order by mnytime" 
'Response.Write(sqlq)
rs_q.Open sqlq,conn_1,3,3,1

if rs_q.EOF then%>
<br>
<table><font class="px12"></font></table>
<%else
%>
   <% do while not rs_q.EOF%>
                   <tr bgcolor="ecf7fd">
                 <td align="middle" ><font color=black class=px12><%=rs_q("mnyyear")%>年<%=month(rs_q("mnytime"))%>月</td>
                <td align="middle" ><font color=black class=px12><%=rs_q("payway")%></td>
                <td align="middle" ><font color=black class=px12><%=rs_q("mnykm")%></td>
                <td align="middle"  ><font color=black class=px12><%=rs_q("price")%></td>
                <td align="middle"  ><font color=black class=px12><%=rs_q("ifhandin")%></td>
                <td align="middle"  ><font color=black class=px12><%=rs_q("mnynote")%></td>
                <td align="middle"  ><font color=black class=px12><%=rs_q("bxname")%></td>

               

                <td align="middle"  ><font color=black class=px12><%=rs_q("passname")%></td>
                <td align="middle"  ><font color=black class=px12><%=rs_q("ifhx")%></td>
                <td align="middle"  ><font color=black class=px12><%=rs_q("changeid")%></td> 
                 <td align="middle"  ><font color=black class=px12><%=rs_q("cz")%></td>
                <td align="middle"  ><font color=black class=px12><%=rs_q("lururen")%></td>
                   <td align="middle"  ><font color=black class=px12><%=rs_q("lurutime")%></td>
