...
function xx(s,y,x) { //v3.0
   //s=depar.value ;
   //y=nian.value ;
   surl="query1.asp?depar="+s+"&passcode="+y+"&bxcode="+x;
...
<table width="740" border="0" cellspacing="0" cellpadding="0" height="90%">
  <tr>
   ...
   set obj.activeConnection=objConn
   if depar="全部" then
   sql="select * from cwys_infoin where (passcode='"&passcode&"' or bxcode='"&bxcode&"') and ifhandin='是' and ifhx='是' order by djdate desc"
   else
   sql="select * from cwys_infoin where (mnydepm='"&depar&"' or passcode='"&passcode&"' or bxcode='"&bxcode&"') and ifhandin='是' and ifhx='是' order by djdate desc"
   end if
   obj.Source=sql
   obj.Open
    %> 
