...
<% 
 ...
 if trim(session("UID"))<>"" then
   ...
   set ObjD=server.CreateObject ("Com_UserManage.ClsUserManage")
       VerifyOk=objD.VerifyUserFunction (session("UID"),"CCS_GSFZCX")
  ...
  set objRst2.activeConnection=objConn
   ...
   bm=trim(Request.QueryString ("bm"))      
    ...
    if VerifyOk_gsfz=true and bm<>"" then
       session("dep")=bm
    else
       bm=depart
       session("dep")=depart
...
function year1_onchange() {
  ...
  sbm='<%=bm%>';
  syear=year1.value ;
  surl='ccs_gsfzcx_index.asp?syear='+syear+'&km='+'<%=km%>';
...
function selbm_onchange() {
  ...
  syear=year1.value ;
  if (sbm=="所有部门")
     surl='ccs_gsfzcx_main.asp?syear='+syear
  else   
          ...
          <td width="605"> 
            <table border="0" cellspacing="0" cellpadding="3" name="menubutton">
              <tr>
                ...
                <td>&nbsp;</td>
                 <td>&nbsp;</td>                 
                 <td><a href="ccs_gsfzcx_main.asp" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapimages('images1','','images/bmcx.gif',1)"><img src="images/bmcx_1.gif" width="100" height="20" border="0" name="images1"></a></td>
                 <td>&nbsp;</td>
                 <td>&nbsp;</td>
                 <td>&nbsp;</td>
                 <td><a href="ccs_gsfzcx_index.asp" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapimages('images4','','images/fycx.gif',1)"><img src="images/fycx_1.gif" width="100" height="20" border="0" name="images4"></a></td>
              </tr>
            </table>
          </td>
        </tr>
...
<table width="740" border="0" cellspacing="0" cellpadding="0" height="90%">
  <tr>
          ...
          <td> 
            <table width="130" border="0" cellspacing="0" cellpadding="1">
                  <%
                    ...
                    sql_fgld="and depar in (select companyname from cwysglbm where ysfgld='"& emid &"')"
                    '副总分管
                    sql="select distinct sx=fkmcode,kem=fkmshuom from cwys_km where nian='"& syear &"' "+sql_fgld+ " order by kem"                   
                    
                    'sql="select distinct sx=fkmcode,kem=fkmshuom from cwys_km where nian='"& syear &"' order by sx"                   
                    objrst.Source =sql
                    objrst.Open 
                    if objrst.EOF and objrst.BOF then
                    else
                     ...
                     <td width="116"><a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=objrst("kem")%>"><font class="px14" color="#FFFFFF"><%=objrst("kem")%></font></a></td>
                     </tr>
                     <%objrst.MoveNext %> 
                       <% wend 
                     end if  
    ...
    <td align=center><a href="ccs_gsfzcx_index.asp?syear=<%=year(date)%>"><font class="px14" color="#FFFFFF">各项统计</font></a></td>
  ...
  %>
   ...
   <tr>
    <td align="center" ><font class="px12" color="black">您目前进入的是分管部门<font color=red><font color=blue><%=km%></font><%=cxsj%></font>预算完成情况页面</td></tr>
    <tr>
    <td align="right" >&nbsp;</td></tr>
     <tr>
    <td align="right" >
  ...
  <tr>
    <td vAlign="top" width="73%">
      <table style="BORDER-RIGHT: #4983a0 1px solid; BORDER-TOP: #4983a0 1px solid; BORDER-LEFT: #4983a0 1px solid; BORDER-BOTTOM: #4983a0 1px solid" height="100%" cellSpacing="0" cellPadding="0" width="100%" border="0">
        ...
        <%' if km="所有科目" then  %>   
        <tr>
          <td style="PADDING-LEFT: 10px; PADDING-TOP: 2px" bgColor="#4983a0" height="20"><font style="COLOR: #ffffff; FONT-FAMILY: Webdings">4</font><font color="#ffffff"><b></b></font> <font color=white><a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=1"><font class="px12" color=white>1月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=2"><font class="px12" color=white>2月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=3"><font class="px12" color=white>3月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=4"><font class="px12" color=white>4月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=5"><font class="px12" color=white>5月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=6"><font class="px12" color=white>6月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=7"><font class="px12" color=white>7月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=8"><font class="px12" color=white>8月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=9"><font class="px12" color=white>9月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=10"><font class="px12" color=white>10月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=11"><font class="px12" color=white>11月</a> <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=12"><font class="px12" color=white>12月   </a>
          <a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd1"><font class="px12" color=white>   第一季度   </a><a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd2"><font class="px12" color=white>第二季度   </a><a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd3"><font class="px12" color=white>第三季度   </a><a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd4"><font class="px12" color=white>第四季度   </a></font><a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=km%>"><font class="px12" color=white>全年   </a></font></td></tr>
            ...
            <table cellSpacing="1" cellPadding="0" width="100%">
              <tbody>
               <tr bgColor="#7dadc4" height="20">
                ...
                <% if km="所有科目" then  %> 
                <td align="middle" ><font class="px12" color="black"><a href="ccs_gsfzcx_index.asp?syear=<%=syear%>&km=<%=trim(objrst("fkmshuom"))%>&month=<%=lb%>"><%=objrst("fkmshuom")%></A></td>
                <% else  %>
   ...
   objrst.Open    
     %>        
                ...
                <td align="middle" ><font class="px12" color="blue"><a href="ccs_gsfzcx_index.asp?syear=<%=syear%>">所有科目</A></td>
