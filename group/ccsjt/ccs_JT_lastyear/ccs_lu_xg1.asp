...
<%objrst1.Source ="select * from cwys_infoin where record_id = '"&q&"'" 
  ...
  objrst2.Source="SELECT fkmcode FROM cwys_km WHERE fkmshuom='"& objrst1("mnykm") &"' and depar='"&f&"' and nian='"&year(date())-1&"'"
  objrst2.open 
...
<br>
    ...
    <font class="px12" color="black">&nbsp;&nbsp;费用期间：</font>
        ...
        <select name="month1" style="HEIGHT: 22px; WIDTH: 43px">
...
<tr><td><font class="px12" color="red">已经提交财务,<%=mnyyear%>年度允许额度<%=yxmy%>元，已经使用<%=mnyused+price%>,还能使用<%=shengyu-price%>元</font><input value="确 定" type="submit" name="action" onclick="javascript:reload()"></td></tr>
</table>
...
<table>
<tr><td><font class="px12" color="blue">数额超标，<font color="red">不能提交财务</font>,<%=mnyyear%>年度<%=jd%>季度允许额度<%=yxmy%>元，已经使用<%=mnyused%>,还能使用<%=yxmy-mnyused%>元,目前需要财务调整额度<%=price-yxmy+mnyused%>元</font><input value="确 定" type="submit" name="action" onclick="javascript:reload()"></td></tr>
</table>
...
<tr><td><font class="px12" color="red">已经提交财务,此次提交成功是由于财务部允许此项费用超标,<%=mnyyear%>年度<%=jd%>季度允许额度<%=yxmy%>元，已经使用<%=mnyused+price%>,还能使用0元<input value="确 定" type="submit" name="action" onclick="javascript:reload()"></font></td></tr>
</table>
<%end if%>
<%end if%> 
<%end sub
