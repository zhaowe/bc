...
<table width="740" border="0" cellspacing="0" cellpadding="0" height="90%">
  <tr>
    ...
    <form method="post" action="ccs_lu_xg1.asp?todo=02&q=<%=q%>" id=form1 name=form1>   
     ...
     <table style="BORDER-RIGHT: #4e4c71 1px solid; BORDER-TOP: #4e4c71 1px solid; BORDER-LEFT: #4e4c71 1px solid; BORDER-BOTTOM: #4e4c71 1px solid" height="92%" cellSpacing="0" cellPadding="0" width="100%" border="0">
        <tbody>
        <tr>
          <td style="PADDING-LEFT: 10px; PADDING-TOP: 2px" bgColor="#cccccc" height="20"><font style="COLOR: #ffffff; FONT-FAMILY: Webdings">4</font><font color="black" class="px12">&nbsp;账单修改</font> </td></tr>
        <tr>
   ...
   <tr>
    <td align="center" ><font class="px12" color="black">您目前进入的是<font color=blue>&nbsp;<%=q%>&nbsp;</font>号纪录<font color=red>&nbsp;<%=objrst("mnykm")%>&nbsp;</font>修改/提交 界面</td></tr>
    <tr>
    <td align="right" >　</td></tr>
...
<tr width=100% bgcolor="white">
     ...
     <input id="hd" type="text" name="hd" size="5" value=<%=objrst("passname")%>>
 ...
 <tr bgcolor="white"><td bgcolor="eeeeee">
     ...
     <input type="text" name="hd1" size="5" value=<%=objrst("bxname")%>>
     
     <font class="px12" color="black">&nbsp;&nbsp;费用科目：</font>
     <input type="text" name="mnykm" size="16" readonly value=<%=objrst("mnykm")%>>

<font class="px12"><%=objrst("mnykmcode")%>
</font>      
...
<br>
    ...
    %>
     ...
     <font class="px12" color="black">&nbsp;&nbsp;付款方式：</font>
        <select size="1" name="payway"><option selected><%=objrst("payway")%>
         ...
         <OPTION  value="冲借支"><font color=black class="px14">冲借支</OPTION>
     ...
     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="price" size="10" value=<%=objrst("price")%>><font class="px12" color="black">元</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font class="px12" color="black">&nbsp;&nbsp;帐单号：</font>
     <input type="text" name="tabid" size="15" value=<%=objrst("tabid")%>><font class="px12" color="black"></font>
    </td>
    </tr>
     <tr bgcolor="white">
     <td  align="left" bgcolor="eeeeee">
     <font class="px12" color="black">&nbsp;&nbsp;费用说明：</font>
               <input type="text" name="mnynote" size="60" value=<%=objrst("mnynote")%>> 
     <br><br> 
    ...
    <tr align="center" bgcolor="white"> <td align="middle" bgcolor="eeeeee"><input type="submit" value="修改/提交" id="submit1" name="submit1"></td></tr> 
        </form>   
...
sqlr="insert into cwys_bmglrz (passcode,passname,bxcode,bxname,djdate,djname,mnydepm,mnykm,mnynote,price,mnytime,payway,ifhandin,ifhx,mnyyear,changeid,cz,lururen,lurutime,mnykmcode) values ('"&passcode&"','"&passname&"','"&bxcode&"','"&bxname&"','"&djdate&"','"&djname&"','"&mnydepm&"','"&mnykm&"','"&mnynote&"','"&price&"','"&date1&"','"&payway&"','"&ifhandin&"','否','"&mnyyear&"','"&q&"','修改','"&session("emid")&"','"&date()&"','"&trim(fkmcode2)&"')"                                                                            
'Response.Write(sqlb)
...
sqlc="SELECT * FROM cwys_ed WHERE fkmcode='"&fkmcode2&"' and depar='"&trim(f)&"' and ys_year='"&trim(mnyyear)&"'" 
'Response.Write(sqlc)                                                                           
...
sqla="update cwys_infoin set passcode='"&passcode&"',passname='"&passname&"',bxcode='"&bxcode&"',bxname='"&bxname&"',djdate='"&djdate&"',mnydepm='"&mnydepm&"',mnykm='"&mnykm&"',mnynote='"&mnynote&"',price="&price&",mnyyear='"&mnyyear&"',mnytime='"&date1&"',payway='"&payway&"',ifhandin='"&ifhandin&"',ifhx='"&ifhx&"',tabid='"&tabid&"' where record_id='"&q&"'"
...
<tr><td><font class="px12" color="red">已经提交</font><input value="确 定" type="submit" name="action" onclick="javascript:reload()"></td></tr>
</table>
...
sqlb="update cwys_infoin set passcode='"&passcode&"',passname='"&passname&"',bxcode='"&bxcode&"',bxname='"&bxname&"',djdate='"&djdate&"',mnydepm='"&mnydepm&"',mnykm='"&mnykm&"',mnynote='"&mnynote&"',price="&price&",mnyyear='"&mnyyear&"',mnytime='"&date1&"',payway='"&payway&"',ifhandin='"&ifhandin&"',ifhx='"&ifhx&"',tabid='"&tabid&"' where record_id='"&q&"'"
'Response.Write(sqlb)
...
<table>
<tr><td><font class="px12" color="blue">数额超标，<font color="red">不能提交</font>,<%=mnyyear%>年度<%=jd%>季度允许额度<%=yxmy%>元，已经使用<%=mnyused%>,还能使用<%=yxmy-mnyused%>元,目前需要财务调整额度<%=price-yxmy+mnyused%>元</font><input value="确 定" type="submit" name="action" onclick="javascript:reload()"></td></tr>
</table>
...
sqlu="update cwys_infoin set passcode='"&passcode&"',passname='"&passname&"',bxcode='"&bxcode&"',bxname='"&bxname&"',djdate='"&djdate&"',mnydepm='"&mnydepm&"',mnykm='"&mnykm&"',mnynote='"&mnynote&"',price="&price&",mnyyear='"&mnyyear&"',mnytime='"&date1&"',payway='"&payway&"',ifhandin='"&ifhandin&"',ifhx='"&ifhx&"',isover='是',tabid='"&tabid&"' where record_id='"&q&"'"
'Response.Write(sqlu)
...
<tr><td><font class="px12" color="red">已经提交,此次提交成功是由于财务部允许此项费用超标,<%=mnyyear%>年度<%=jd%>季度允许额度<%=yxmy%>元，已经使用<%=mnyused+price%>,还能使用0元<input value="确 定" type="submit" name="action" onclick="javascript:reload()"></font></td></tr>
</table>
<%end if%>
<%end if%> 
<%end sub
