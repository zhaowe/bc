...
if trim(session("UID"))<>"" then
   ...
   set ObjD=server.CreateObject ("Com_UserManage1.ClsUserManage1")
       VerifyOk=objD.VerifyUserFunction (session("UID"),"ccs_gscn_cx")
...
function xx(s,y,x,a,b,c,d) { //v3.0
   //s=depar.value ;
   //y=nian.value ;
   surl="cwmain_cx.asp?depar="+s+"&passcode="+y+"&bxcode="+x+"&payway="+a+"&price="+b+"&tabid="+c+"&qijian="+d;
          ...
          <td width="605"> 
            <table border="0" cellspacing="0" cellpadding="3" name="menubutton">
              <tr>
                <td>&nbsp;</td>
                 <td>&nbsp;</td>
                <td><a href="cwmain_cx.asp" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapimages('images1','','images/baoxiao1.gif',1)"><img src="images/baoxiao.gif" width="120" height="24" border="0" name="images1"></a></td>
...
<table width="740" border="0" cellspacing="0" cellpadding="0" height="90%">
  <tr>
        ...
        <tr> 
          <td> 
            <table width="130" border="0" cellspacing="0" cellpadding="1">
                     ...
                     <td><a href="cwmain_cx.asp"><font class="px14" color="red">未核销查询</font></a></td>
                      </tr>
                      
                      <tr>
                      <td><img src="images/a.gif" WIDTH="28" HEIGHT="28"></td>
                      <td><a href="query1_cncx.asp"><font class="px14" color="#FFFFFF">已核销查询</font></a></td>
                       </tr>
    ...
    <td width="595" valign="top"> 
     ...
     <td >    <font class="px12" color="black">&nbsp;&nbsp;部门：<select name="depar" style="HEIGHT: 22px; WIDTH: 150px"> 
   ...
   obj.Open
    ...
    <option><%=year(date)+2%></option>
     ...
     <select name="qijian">
     	   <OPTION  selected value="<%=month(now)%>"><font color=black class="px14"><%=month(now)%></OPTION>
         <OPTION  value="1"><font color=black class="px14">1</OPTION>
         <OPTION  value="2"><font color=black class="px14">2</OPTION>
         <OPTION  value="3"><font color=black class="px14">3</OPTION>
         <OPTION  value="4"><font color=black class="px14">4</OPTION>
         <OPTION  value="5"><font color=black class="px14">5</OPTION>
         <OPTION  value="6"><font color=black class="px14">6</OPTION>
         <OPTION  value="7"><font color=black class="px14">7</OPTION>
         <OPTION  value="8"><font color=black class="px14">8</OPTION>
         <OPTION  value="9"><font color=black class="px14">9</OPTION>
         <OPTION  value="10"><font color=black class="px14">10</OPTION>
         <OPTION  value="11"><font color=black class="px14">11</OPTION>
         <OPTION  value="12"><font color=black class="px14">12</OPTION>
     </select>月
     ...
     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
     <input type="button" name="action" value="查询" onclick="javascript:xx(depar.value,passcode.value,bxcode.value,payway.value,price.value,tabid.value,qijian.value)">
     </td>
     </tr>             
    </table>
      ...
      if nian<>"" then
      sql7="and year(mnytime)='"&nian&"'"
      end if 
         
      qijian=trim(Request.QueryString("qijian"))
      if qijian<>"" then
      sql8="and month(mnytime)='"&qijian&"'"
      end if    
      %> 
    <%
   Set objConn = Server.CreateObject("ADODB.Connection")
   ...
   obj.CursorType=3
   set obj.activeConnection=objConn
   sql="select * from cwys_infoin where ifhandin='是' and ifhx='否'"+sql1+sql2+sql3+sql4+sql5+sql6+sql7+sql8+"order by djdate desc"
   obj.Source=sql
   obj.Open
    %> 
   <% if not obj.eof then%>  
     ...
     <td ><font class="px12" color="black">报销单号</font></td>
     </tr>   
   <%
   obj.MoveFirst 
   while not obj.EOF
   ...
   <%'else%>
     ...
     <td ><font class="px12" color="black"><%=obj("tabid")%></font></td>
     </tr>
    <%'end if%>  
    <%'obj1.Close%> 
