...
<% 
 ...
 if trim(session("UID"))<>"" then
   ...
   set ObjD=server.CreateObject ("Com_UserManage1.ClsUserManage1")
       VerifyOk=objD.VerifyUserFunction (session("UID"),"CCS_gkbmcx")
  ...
  set objRst2.activeConnection=objConn
   ...
   bm=trim(Request.QueryString ("bm"))      
    ...
    if VerifyOk_gkbmcx=true and bm<>"" then
       session("dep")=bm
    else
       bm=depart
       session("dep")=depart
...
<html>
<head>
<title>预算控制系统--<%="按费用查询"%><%=depart%></title>
...
function year1_onchange() {
  ...
  sbm='<%=bm%>';
  syear=year1.value ;
  surl='ccs_gkbmcx_index.asp?syear='+syear+'&km='+'<%=km%>';
...
function selbm_onchange() {
  ...
  syear=year1.value ;
  if (sbm=="所有部门")
     surl='ccs_gkbmcx_main.asp?syear='+syear
  else   
     surl='ccs_gkbmcx_bmldcx_main.asp?syear='+syear+'&bm='+sbm;
  window.document.location.href(surl);
}
...
<table width="740" border="0" cellspacing="0" cellpadding="0" height="90%">
  <tr>
        ...
        <tr> 
          <td> 
           ...
           <%'小黄开始%>            
                     ...
                     Set objrst_kmlb=server.CreateObject ("ADODB.Recordset")
                     objrst_kmlb.LockType=3
                     objrst_kmlb.CursorType=3
                     set objrst_kmlb.activeConnection=objConn
                    
                    ' objrst_kmlb.Source ="select distinct kmshuom,kmcode from cwys_km where nian='"& syear &"' and kmshuom is not null order by kmcode,kmshuom " 
                     objrst_kmlb.Source ="SELECT DISTINCT cwys_km.fkmshuom, cwys_km.fkmcode FROM cwys_km INNER JOIN cwys_gkbm ON cwys_km.fkmcode = cwys_gkbm.fkmcode WHERE cwys_gkbm.department = '"& trim(depart) &"' ORDER BY cwys_km.fkmshuom" 
          ...
          <TR>
            ...
            <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=objrst_kmlb("fkmshuom")%>"> <font class="px14" color="#FFFFFF"><%=trim(objrst_kmlb("fkmshuom"))%></font></a>
                  </TD>
    ...
    <td align=center><a href="ccs_gkbmcx_index.asp?syear=<%=year(date)%>"><font class="px14" color="#FFFFFF">各项统计</font></a></td>
  ...
  %>
   ...
   <tr>
    <td align="center" ><font class="px12" color="black">您目前进入的是<font color=red><font color=blue><%=km%></font><%=cxsj%></font>预算完成情况页面</td></tr>
    <tr>
    <td align="right" >&nbsp;</td></tr>
     <tr>
    <td align="right" >
   ...
   sql_he=sql_he+" from cwys_ed as a,(SELECT DISTINCT cwys_km.fkmshuom as fkmshuom, cwys_km.depar as mnydepm , cwys_km.fkmcode as fkmcode FROM cwys_km INNER JOIN cwys_gkbm ON cwys_km.fkmcode = cwys_gkbm.fkmcode WHERE cwys_gkbm.department = '"& trim(depart) &"') as b  " 
   ...
   sql_he=sql_he+" and a.fkmcode=b.fkmcode and a.ys_year='"& syear &"' and a.depar=b.mnydepm "      
   
   sql_bmkm="left join cwys_infoin b on a.fkmcode=b.mnykmcode and a.depar=b.mnydepm and a.ys_year='"& syear &"' and mnyyear='"& syear &"'  and ifhandin='是' and cz<>'删除'  "  
   ...
   sql=sql+" ) as a " 
   'sql=sql+"left join cwys_infoin b on a.fkmcode=b.mnykmcode and a.ys_year=b.mnyyear  and ifhandin='是' and cz<>'删除'  "
   sql=sql+sql_bmkm
   sql=sql+sql_fy 
   sql=sql+") as c " 
  ...
  <tr>
    <td vAlign="top" width="73%">
      <table style="BORDER-RIGHT: #4983a0 1px solid; BORDER-TOP: #4983a0 1px solid; BORDER-LEFT: #4983a0 1px solid; BORDER-BOTTOM: #4983a0 1px solid" height="100%" cellSpacing="0" cellPadding="0" width="100%" border="0">
       ...
       <%' if km="所有科目" then  %>   
        <tr>
          <td style="PADDING-LEFT: 10px; PADDING-TOP: 2px" bgColor="#4983a0" height="20"><font style="COLOR: #ffffff; FONT-FAMILY: Webdings">4</font><font color="#ffffff"><b></b></font> <font color=white><a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=1"><font class="px12" color=white>1月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=2"><font class="px12" color=white>2月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=3"><font class="px12" color=white>3月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=4"><font class="px12" color=white>4月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=5"><font class="px12" color=white>5月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=6"><font class="px12" color=white>6月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=7"><font class="px12" color=white>7月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=8"><font class="px12" color=white>8月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=9"><font class="px12" color=white>9月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=10"><font class="px12" color=white>10月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=11"><font class="px12" color=white>11月</a> <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=12"><font class="px12" color=white>12月   </a>
          <a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd1"><font class="px12" color=white>   第一季度   </a><a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd2"><font class="px12" color=white>第二季度   </a><a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd3"><font class="px12" color=white>第三季度   </a><a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd4"><font class="px12" color=white>第四季度   </a></font><a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=km%>"><font class="px12" color=white>全年   </a></font>
            ...
            <table cellSpacing="1" cellPadding="0" width="100%">
              <tbody>
               <tr bgColor="#7dadc4" height="20">
                ...
                <% else  %>                             
                <td align="middle"  ><font class="px12" color="black">部门名称</td>                                
                <% end if  %>                                             
                ...
                <% if km="所有科目" then  %> 
                <td align="middle" ><font class="px12" color="black"><a href="ccs_gkbmcx_index.asp?syear=<%=syear%>&km=<%=trim(objrst("fkmshuom"))%>&month=<%=lb%>" target=_blank><%=objrst("fkmshuom")%> </A></td>
                <% else  %>
                <td align="middle" ><font class="px12" color="black"><a href="ccs_gkbmcx_bmldcx_index.asp?syear=<%=syear%>&bm=<%=trim(objrst("depar"))%>&km=<%=trim(objrst("fkmshuom"))%>&month=<%=lb%>" target=_blank><%=objrst("depar")%></A></td>
                <% end if  %>
   ...
   objrst.Open    
     %>        
                ...
                <td align="middle" ><font class="px12" color="blue"><a href="ccs_gkbmcx_index.asp?syear=<%=syear%>">所有科目</A></td>
...
<div id="Div5" style="Z-INDEX: 101; LEFT: 840px; POSITION: absolute; TOP:233px;display:none">
 <table width=86px border=0 cellpadding=0 cellspacing=0> 
  ...
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=1">一月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=2">二月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=3">三月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=4">四月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=5">五月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=6">六月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=7">七月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=8">八月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=9">九月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=10">十月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=11">十一月预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=12">十二月预算报表</a></span></td></tr>
  ...
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=jd1">一季度预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=jd2">二季度预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=jd3">三季度预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>&month=jd4">四季度预算报表</a></span></td></tr>
  <tr><td valign="top" width="86px" bgcolor=gray style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"></span></td></tr>
    <tr><td valign="top" width="86px" bgcolor=#cccccc style="font-size:12px;font-family:宋体color：black;height:18px;filter:Alpha(opacity=100);border-left:1px solid #ffffff;border-right:1px solid #ffffff;" onmouseover="this.className='td_mouseover';" onmouseout="this.className='td_mouseout';"><span style="position:absolute"><a href="ccs_gkbmcx_index_excel.asp?syear=<%=syear%>&km=<%=km%>">全年预算报表</a></span></td></tr>
