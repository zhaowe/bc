...
Sub btnStartQuery_onclick
	frmOperate.action = "Operate.asp?strField=" & frmOperate.selField.value & "&strFlag=" & frmOperate.selFlag.value & "&strValue=" & frmOperate.txtValue.value
	frmOperate.submit
End Sub
-->
</SCRIPT>
...
If Request.QueryString("strField") = "" Then
	Set objRs = objDML.ErrorQuery("UserId",3,Session("UserId"))  '取登录用户添加的记录
	'set objRs = objDML.ErrorQuery()
Else
	'strFlag取值:=(1),>(2),like(3),<(4)
	Dim iFlag
	iFlag = cint(Request.QueryString("strFlag"))
	set objRs = objDML.ErrorQuery(Request.QueryString("strField"),iFlag,Request.QueryString("strValue"))
End If
...
%> 
		<tr>
		<td align="center" width="5%"><a href="EditError.asp?ErrNo=<%=objRs("ErrorNo")%>"><%=objRs("ErrorNo")%></td>
		...
		<td align="center" width="8%"><%=objRs("LastModify")%></td>
		</tr>
<%	
		objRs.movenext
		If objRs.EOF Then
...
set objDML=nothing
%>
<A href="Restore.asp">恢复纪录</A>&nbsp;&nbsp;<A href="AddError.asp">添加纪录</A>&nbsp;&nbsp;<A href="Login.asp">重新登录</A>
</form>
</head></html>