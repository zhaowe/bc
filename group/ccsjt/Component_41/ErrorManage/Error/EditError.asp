...
%>
&nbsp;&nbsp;&nbsp;
<a href="DelError.asp?ErrNo=<%=paraErrNo%>">删除</a>
&nbsp;&nbsp;&nbsp;
<a href="Operate.asp">返回主页面</a>
<%
Dim strSql
...
%>
<Form id=frmEdit Method="Post" Action="SaveErr.asp">
...
<div align="center">
  ...
  <table border="1" width="90%" height="120" cellspacing="1" cellpadding="10">
	...
	objRs.movenext
	loop%>
<center><a href="ShowSub.asp?Operate=add&ErrNo=<%=paraErrNo%>">添加子表纪录</a>
&nbsp;&nbsp;
<Input Type="Submit" name="btnSubmit" value="提交"></center>
</form>
...
%>
	<tr>
		<td width="10%">
		<%if objRs("LocaleType") <> "zh" or objRs("ProtocolType")="wap" then%>
		<a href="showsub.asp?Operate=edit&ErrNo=<%=objRs("ErrorNo")%>&Locale=<%=objRs("LocaleType")%>&Protocol=<%=objRs("ProtocolType")%>">
		<%end if%>
		<%=objRs("ErrorNo")%></td>
