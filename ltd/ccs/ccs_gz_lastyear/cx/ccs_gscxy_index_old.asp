...
function year1_onchange() {
  ...
  sbm='<%=bm%>';
  syear=year1.value ;
  surl='ccs_gsldcx_index.asp?syear='+syear+'&km='+'<%=km%>';
        ...
        <tr> 
          <td> 
            <table width="130" border="0" cellspacing="0" cellpadding="1">
                  <%objrst.Source ="select distinct kmcode,sx=fkmcode,kem=fkmshuom from cwys_km where nian='"& syear &"' order by kmcode,sx" 
                  'objrst.Source ="select distinct kem,sx from shenzhencwys_dep where dep='"& session("dep") &"' order by sx" 
                    ...
                    if objrst.EOF and objrst.BOF then
                     ...
                     <td width="116"><a href="ccs_gscxy_index.asp?syear=<%=syear%>&km=<%=objrst("kem")%>"><font class="px14" color="#FFFFFF"><%=objrst("kem")%></font></a></td>
                     </tr>
                     <%objrst.MoveNext %> 
                       <% wend 
                     end if  
...
<%' if VerifyOk_gsld=true then %>
      ...
      <FORM action="ccs_gscxy_index_Kmcode_cx.asp" method=POST id=form1 name="Form_Kmcode">

      <font class="px12" color="red">请输入科目一级代码</font>&nbsp;
      <INPUT type="text" id=text1 name="Kmcode_1">
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <font class="px12" color="red">请输入科目二级代码</font>&nbsp;                    
      <INPUT type="text" id=text1 name="Kmcode_2">
...
<%' end if  %>        
   ...
   if km="所有科目" then 
     sql_1="select ys_year,kmcode,fkmcode,fkmshuom,ze,je=sum(je),bl=sum(je)/ze from " 
     sql_km=""
     sql_gb="group by ys_year,kmcode,fkmcode,fkmshuom,ze  " 
   
   sql_bmkm="left join cwys_infoin b on a.fkmcode=b.mnykmcode and a.ys_year=b.mnyyear  and ifhandin='是' and cz<>'删除'  "  
   
   sql_he=" select a.ys_year,b.kmcode,a.fkmcode,ze,b.fkmshuom "
   sql_he=sql_he+ " from (select ys_year,fkmcode,ze=sum("
   sql_he=sql_he+sql_ze 
   sql_he=sql_he+ " ) from cwys_ed as a where ys_year='"& syear &"' group by ys_year,fkmcode) as a,(select distinct kmcode,fkmcode,fkmshuom,nian from cwys_km) as b "
   sql_he=sql_he+ " where a.fkmcode=b.fkmcode and a.ys_year=b.nian "
   sql_sort=" order by kmcode,fkmcode,fkmshuom "
     
   else
     sql_1="select ys_year,depar,fkmcode,fkmshuom,ze,je=sum(je),bl=sum(je)/ze from " 
     sql_km=" and b.fkmshuom='"& km &"' "
     sql_gb="group by ys_year,depar,fkmcode,fkmshuom,ze  " 
   ...
   sql_sort=" order by fkmcode,fkmshuom "
  ...
  <tr>
    <td vAlign="top" width="73%">
      <table style="BORDER-RIGHT: #4983a0 1px solid; BORDER-TOP: #4983a0 1px solid; BORDER-LEFT: #4983a0 1px solid; BORDER-BOTTOM: #4983a0 1px solid" height="100%" cellSpacing="0" cellPadding="0" width="100%" border="0">
        ...
        <tr>
          ...
          <a href="ccs_gscxy_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd1"><font class="px12" color=white>   第一季度   </a><a href="ccs_gscxy_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd2"><font class="px12" color=white>第二季度   </a><a href="ccs_gscxy_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd3"><font class="px12" color=white>第三季度   </a><a href="ccs_gscxy_index.asp?syear=<%=syear%>&km=<%=km%>&month=jd4"><font class="px12" color=white>第四季度   </a></font><a href="ccs_gscxy_index.asp?syear=<%=syear%>&km=<%=km%>"><font class="px12" color=white>全年   </a></font></td></tr>
            ...
            <table cellSpacing="1" cellPadding="0" width="100%">
              <tbody>
               <tr bgColor="#7dadc4" height="20">
                ...
                <% else  %>
                <td align="middle" ><font class="px12" color="black"><a href="ccs_bmldcx_main.asp?syear=<%=syear%>&bm=<%=trim(objrst("depar"))%>&month=<%=lb%>"><%=objrst("depar")%></A></td>
                <% end if  %>
   ...
   objrst.Open    
     %>        
                ...
                <% if km="所有科目" then  %> 
                <td align="middle" ><font class="px12" color="blue"></td>
                <td align="middle" ><font class="px12" color="blue"></td>
                <% end if  %>
