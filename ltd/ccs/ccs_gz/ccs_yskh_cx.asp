...
<table width="740" border="0" cellspacing="0" cellpadding="0" height="90%">
        ...
        <tr> 
          <td> 
            <table width="130" border="0" cellspacing="0" cellpadding="1">
                     ...
                     <td><a href="ccs_yskh_index.asp"><font class="px14" color="#FFFFFF">预算录入及修改</font></a></td>
                      </tr>
                      <tr>
                      <td><img src="images/a.gif" WIDTH="28" HEIGHT="28"></td>
                      <td><a href="ccs_yskh_cx.asp"><font class="px14" color="#FFFFFF">预算查询</font></a></td>
                       </tr>
                        <tr>
                      <td><img src="images/a.gif" WIDTH="28" HEIGHT="28"></td>
                      <td><a href="ccs_yskh_del.asp"><font class="px14" color="#FFFFFF">预算删除</font></a></td>
                       </tr>
  ...
  <tbody>
   ...
   <tr>
    <td align="center"><font class="px12" color="black">您目前查询的是<font color="red"> <%=year(date)%>年<%=session("dep")%><%=k%>财务预算核定数</font></td></tr>
      <tr>
    <td align="right">&nbsp;</td></tr>   
      <%riqi=year(date)%>
      <%redim datet(12)
    ...
    &nbsp;
     ...
     <td>
    <font class="px12" color="black"> 一月份
     </td>
     <td>
     <font class="px12" color="black">二月份
     </td>
     <td>
     <font class="px12" color="black">三月份
     </td>
     <td>
     <font class="px12" color="black">四月份
     </td>
     <td>
     <font class="px12" color="black">五月份
     </td>
     <td>
     <font class="px12" color="black">六月份
     </td>
     </tr>
        <tr>
         <td>
    &nbsp;
      ...
      <td>
      <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(1) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     &nbsp;
     <%end if
     objrst.Close 
     %>
      ...
      <td>
   <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(2) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     %>
      ...
      <td>
     <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(3) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     %>
      ...
      <td>
      <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(4) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     %>
      ...
      <td>
    <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(5) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     %>
      ...
      <td>
      <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(6) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
    ...
    &nbsp;
     ...
     <td>
     <font class="px12" color="black">七月份
     </td>
     <td>
     <font class="px12" color="black">八月份
     </td>
     <td>
     <font class="px12" color="black">九月份
     </td>
     <td>
     <font class="px12" color="black">十月份
     </td>
     <td>
     <font class="px12" color="black">十一月份
     </td>
      <td>
     <font class="px12" color="black">十二月份
     </td>
     </tr>
    ...
    <tr>
     ...
     <td>
    <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(7) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     %>
      ...
      <td>
     <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(8) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     %>
      ...
      <td>
  <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(9) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     %>
      ...
      <td>
     <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(10) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     <td>
     <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(11) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
     ...
     <td>
    <%objrst.Source ="select * from shenzhencwys_je where kem='"& k &"' and yea='"& datet(12) &"' and dep='"& session("dep") &"'"
      objrst.Open%>
     <%if not objrst.BOF then%>
     <input type="text" name="m7" size="8" value=<%=objrst("mon")%>>    
     <%else%>
   ...
   <tr>
    <td>
     <form method="post" action="ccs_yskh_cx_cl.asp" id="form1" name="form1">   
    <table>
      <tr><td align="right">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  <font class="px12" color="black">请选择财务预算年份&nbsp;<select name="year1" style="HEIGHT: 22px; WIDTH: 57px"> <option selected><%=year(date)%></option>
     <option>2007</option><option>2008</option><option>2009</option><option>2010</option></select>
      <font color="#ff0000"></font>&nbsp;&nbsp;&nbsp;<font class="px12" color="black">请选择财务预算项目&nbsp;<select name="kem" style="HEIGHT: 22px; WIDTH: 77px">
          <%objrst.Source ="select distinct khkem from shenzhencwys_khkem where dep='"& session("dep") &"'" 
                       objrst.Open 
                       objrst.MoveFirst
                       while not objrst.EOF %>
                        <option><%=objrst("khkem")%></option> 
                      <%objrst.MoveNext %> 
                       <% wend %>
                         </select>
