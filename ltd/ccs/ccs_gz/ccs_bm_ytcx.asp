...
function xx(j,k,p) { //v3.0
   //s=depar.value ;
   //y=nian.value ;
   surl="ccs_bm_ytcx.asp?mnykm="+j+"&year1="+k+"&mnykmcode="+p;
        ...
        <tr>
          ...
          <td align="right">
            <table border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td><a href><img src="images/bmysglsm4.gif" width="138" height="28" border="0"></a></td>
              </tr>
            </table>
          </td>
        </tr>
  ...
  set objRst.activeConnection=objConn
                  ...
                  <TR>
                    <TD style="PADDING-LEFT: 0px" height=23> <A title=资料册 
                        href="ccs_bm_ytcx.asp?km=<%=trim(objrst("fkmshuom"))%>&km1=<%=trim(objrst("fkmcode"))%>&km2=<%=trim(objrst("kmshuom"))%>" 
    ...
    <td width="595" valign="top"> 
     <table style="BORDER-RIGHT: #4e4c71 1px solid; BORDER-TOP: #4e4c71 1px solid; BORDER-LEFT: #4e4c71 1px solid; BORDER-BOTTOM: #4e4c71 1px solid" height="92%" cellSpacing="0" cellPadding="0" width="100%" border="0">
        <tbody>
        <tr>
          <td style="PADDING-LEFT: 10px; PADDING-TOP: 2px" bgColor="#4e4c71" height="20"><font style="COLOR: #ffffff; FONT-FAMILY: Webdings">4</font><font color="#ffffff" class="px14">&nbsp;预提查询</font> </td></tr>
        <tr>
   ...
   <tr>
    <td align="center" ><font class="px12" color="black">您目前进入的是<font color=blue><%=km2%>(<%=km%>) </font><font color=red>预提查询</font>界面</td></tr>
    <tr>
    <td align="right" >&nbsp;</td></tr>
...
sql4="select sum(price) as priceall from cwys_infoin where payway='预提' and mnykmcode='"&fkmcode1&"' and mnyyear='"&mnyyear1&"' and mnydepm='"&f&"' group by payway"                                                                            
rs_4.Open sql4,conn_1,3,3,1
'Response.Write(sql4)

set rs_5=server.CreateObject("adodb.recordset")                                                                            
rs_5.CursorLocation=2                                                                            
sql5="select sum(price) as priceused from cwys_infoin where payway='冲销' and mnykmcode='"&fkmcode1&"' and mnyyear='"&mnyyear1&"' and mnydepm='"&f&"' group by payway"                                                                            
rs_5.Open sql5,conn_1,3,3,1
'Response.Write(sql5)
if rs_5.EOF then
a=0
else
...
'sql7="select * from cwys_infoin where payway='预提' and mnykm='"&mnykm&"' and mnyyear='"&mnyyear1&"' and mnydepm='"&f&"'" 
sql7="select sum(price)as price,mnytime,payway,mnykm,djname,djdate from cwys_infoin where (payway='预提' or payway='冲销') and mnykmcode='"&fkmcode1&"' and mnyyear='"&mnyyear1&"' and mnydepm='"&f&"' group by mnytime,payway,mnykm,djname,djdate order by mnytime,payway desc" 
...
<%else
%>
<table><font class="px12" color="blue"><%=f%>  / <%=mnykm%> / <%=mnyyear1%>年度 / 预提<%=rs_4("priceall")%>元 已经冲销<%=0-a%>元</font></table>
 <br>
<table bgcolor="white" width="750" cellSpacing="1" cellPadding="0">

       <tr bgColor="#7dadc4" height="20">
                <td align="middle"  ><font color=black class=px12><b>预提月份</b></td>
