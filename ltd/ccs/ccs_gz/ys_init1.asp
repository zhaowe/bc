...
<!--
function edit(nian,kemu,edu,depar)
{
tzurl="ys_init2.asp?nian="+nian+"&depar="+depar+"&kemu="+kemu+"&edu="+edu;
...
fenpei=trim(Request.form("fenpei"))
...
<%
   ...
   sql="select * from cwys_ed where depar='"&depar&"' and fkmcode='"&kemu&"' and ys_year='"&nian&"'"
   obj.Source=sql
   obj.Open
%>   
<% if not obj.EOF  then %> 
<tr align="center" >
<td align="center" >
	<font class=px14 color=blue>该项预算指标已存在，请不要重复导入，如需改动请点击预算管理。</font></p>
	<input type="button" name="button" value="返回" onclick="JavaScript:history.go(-1)">
</td>
</tr>
<%
...
descr="将"+depar+nian+"年的"+kemu+"预算"+edu+"元，平均分配给各月。"
set rs_b=server.CreateObject("adodb.recordset")                                                                            
rs_b.CursorLocation=2  
sqlb="insert into cwys_log (operation,descr,type,operatetime,operator) values ('增加','"&descr&"','预算','"&now&"','"&session("emid")&"')" 
...
<tr align="center" >
<td align="center" >
	<font class=px14 color=blue>已经将预算指标按月平分，导入成功。</font></p>
	<input type="button" name="button" value="返回" onclick="JavaScript:history.go(-1)">
</td>
</tr>
<%else%>
<script>edit("<%=nian%>","<%=kemu%>","<%=edu%>","<%=depar%>");</script>
<%end if%>
<%end if%>
<%
end if
